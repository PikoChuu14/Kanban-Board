...
function ManagerDashboard({ user, refreshKey, onViewKanban, onViewProject, onOpenKanban }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) { setWorkload(team); setTasks(staffTasks.flat()); setBoards(teamBoards); setMyTasks(ownTasks); setMySnapshot(startOfDay); setState("ready"); }
  ...
  return <section className="dashboard-page">
    <div className="dashboard-hero"><div><h1 className="personal-greeting">{timeGreeting(user.name)}</h1><p className="greeting-subtitle">Here's what's happening with your team today.</p><p className="eyebrow">Manager dashboard</p><h2>Team operations</h2><p>Your work and your department’s operational picture.</p></div></div>
    ...
    <div className="dashboard-columns manager-grid">
      <DashboardPanel title="Needs my attention"><div className="attention-summary"><span><strong>{review.length}</strong> waiting for review</span><span><strong>{overdue.length}</strong> overdue</span><span><strong>{dueSoon.length}</strong> due soon</span></div>{attention.map((task) => <div className="dashboard-task attention-task" key={task.id}><div><strong>{task.title}</strong><small>{task.assigneeName || "Unassigned"} · {task.boardName || "No project"}{task.dueDate ? ` · ${task.dueDate}` : ""}</small></div><div className="attention-actions"><div className="attention-badges">{task.status === "REVIEW" && <span className="attention-badge review">Waiting for review</span>}{dueLabel(task) && <span className={`attention-badge ${dueLabel(task) === "Overdue" ? "overdue" : "deadline"}`}>{dueLabel(task)}</span>}</div>{task.status === "REVIEW" && <><button className="approve-button" onClick={() => reviewAction(task, "APPROVE")}>Approve</button><button className="return-button" onClick={() => reviewAction(task, "RETURN")}>Return</button></>}</div></div>)}{!attention.length && <Empty text="No tasks require your attention." />}</DashboardPanel>
